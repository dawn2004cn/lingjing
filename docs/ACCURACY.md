...
- 紫微主站 `ziweiSchool=ni|feixing`；飞星含大限四化/自化/来因宫/**本命飞化链（落宫+再飞一跳）**
- 首页追问支持多轮线程；排盘与规则事实不限，LLM 计日额度
- 会员档：`free` / `pro`；兑换码 + **订单/演示支付骨架**（`provider=mock`）；微信/支付宝通道未接
- citation：词表启发式 + 宫位加权 + **语义 lite**（星入宫/日主日柱断言对照盘面）；非完整 NLP
- **产品级未做（与术数深度解耦）**：微信/支付宝正式收款、完整语义事实图校验、大限/流年多层飞化长链
