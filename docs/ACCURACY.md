...
- 紫微主站 `ziweiSchool=ni|feixing`；飞星含本命/大限/流年/**流月**多层飞化链、自化、来因宫
- 首页追问支持多轮线程；排盘与规则事实不限，LLM 计日额度
- 会员档：`free` / `pro`；兑换码 + **订单/演示支付骨架**（`provider=mock`）；微信/支付宝通道未接
- citation：**语义事实图**（星入宫、本命四化、化忌落宫、命宫支、对宫、格局名、八字日主/柱/喜用）+ 词表加权；合盘互飞落点并入事实索引
- 合盘：关键宫对照 + 四化互飞 + **落宫再飞一跳 / 关键宫落点语义图**
- **产品级未做（与术数深度解耦）**：微信/支付宝正式收款；流日叠链 / 独立会话产品仍可加深
