...
- 紫微主站已支持 `ziweiSchool=ni|feixing`（表单开关；飞星输出大限宫干四化/自化/本命四化来因宫）
- 首页追问支持多轮线程（保留主解读，追加问答，服务端带最近历史）
- citation：星曜/干支启发式 + **宫位名加权**（幻觉宫位权重×2）；仍非语义级事实校验
- **产品级未做（与术数深度解耦）**：付费档、citation 语义级事实校验
